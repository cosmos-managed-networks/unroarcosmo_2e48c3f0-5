...
HTTPS_GIT := github.com/unroard/unroarcosmo.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=unroarcosmo \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=unroard \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/unroard ./cmd/unroard
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/unroard.exe ./cmd/unroard
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/unroard
...
draw-deps:
	...
	@goviz -i ./cmd/unroard -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c unroarcosmo --local -f chains.yaml
endif
...
set-testnet-configs:
	unroard config set client chain-id chainid-1
	unroard config set client keyring-backend test
	unroard config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | unroard keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | unroard keys add acc1 --recover`
