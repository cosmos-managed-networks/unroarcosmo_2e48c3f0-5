...
# then log output of file /code/bin/unroard
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/unroard \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/unroard | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/unroard /usr/bin/unroard
